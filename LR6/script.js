...
function rotorForward(letter, rotor) {
    ...
    let newLetter = rotor.array[index + rotor.position];
    let newIndex = ALPH.indexOf(newLetter);
    return ALPH[newIndex];
}

function rotorBack(letter, rotor) {
    let index = rotor.array.indexOf(letter);
    let newLetter = ALPH[index - rotor.position];
    let newIndex = rotor.array.indexOf(newLetter);
    return rotor.array[newIndex];
}
